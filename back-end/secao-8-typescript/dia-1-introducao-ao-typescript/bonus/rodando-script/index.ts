...
class Conta {
  private nomeTitular: string // atributo privado
  private saldo: number // atributo privado
  private ativa: boolean = true // atributo privado
  
  constructor({ //classes precisam de constructor
    nomeTitular,
    saldo = 0
  }: IConta){
  ...
  depositar = (valor: number): void | boolean => { // recebe um valor numerico e não retorna nd ou retorna um boolean
    if(!this.ativa){
...
const conta: Conta = new Conta({nomeTitular: 'Nath'}) // instanciando um outro obj
